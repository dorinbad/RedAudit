...
class TestExpandUserPath(unittest.TestCase):
    def test_expand_user_path_under_sudo_uses_invoking_user_home(self):
        with patch.dict(os.environ, {"SUDO_USER": "alice"}, clear=False), patch(
            "redaudit.utils.paths.os.geteuid", return_value=0
        ), patch(
...
class TestDocumentsDir(unittest.TestCase):
    def test_get_documents_dir_prefers_xdg_user_dirs(self):
        ...
        with patch("redaudit.utils.paths.open", mock_open(read_data=content)), patch(
            "redaudit.utils.paths.os.path.isabs", return_value=True
        ):
    ...
    def test_get_documents_dir_falls_back_to_existing_documentos(self):
        ...
        with patch("redaudit.utils.paths.open", side_effect=FileNotFoundError), patch(
            "redaudit.utils.paths.os.path.isdir", side_effect=isdir_side_effect
        ):
            self.assertEqual(paths.get_documents_dir("/home/alice"), "/home/alice/Documentos")
...
class TestChownTree(unittest.TestCase):
    def test_maybe_chown_tree_calls_chown(self):
        with patch("redaudit.utils.paths.resolve_invoking_user_owner", return_value=(1000, 1000)), patch(
            "redaudit.utils.paths.os.chown"
        ) as mock_chown, patch(
